	...
	constructor(clientId, websocket) {
		...
		this.message = (function message(type, message, status) {
			...
			if (type === "roomActionState") {
				...
				try {
					...
					if (!lastSent || Date.now() - lastSent > 60*1000) {
						lastSent = Date.now()
						this.getRoom().messageAll([this.clientId], "roomActionPlaceTiles", `${this.getNickname()} has encountered an error, which affects this turn, and possibly successive ones. You can manually control the bot <a target="_blank" href="#clientId=${this.clientId}">here</a>. This message will be sent for the first error every minute. `, "error")
					}
				}
			}
			...
			if (status === "error" && message.includes && !message.includes("manually control the bot") && !message.includes("higher priority placement")) {
				...
				if (!lastSent || Date.now() - lastSent > 60*1000) {
					lastSent = Date.now()
					this.getRoom().messageAll([this.clientId], "roomActionPlaceTiles", `${this.getNickname()} has received an error message. If it is not functioning, you can manually control the bot <a target="_blank" href="#clientId=${this.clientId}">here</a>. This message will be sent for the first error every minute. `, "error")
				}
